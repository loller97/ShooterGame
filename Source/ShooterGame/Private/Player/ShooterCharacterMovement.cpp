...
void UShooterCharacterMovement::BeginPlay()
{
	...
	if (GetPawnOwner()->GetLocalRole() > ROLE_SimulatedProxy)
	{
		...
		static_cast<AShooterCharacter*>(GetCharacterOwner())->GetCapsuleComponent()->OnComponentHit.AddDynamic(this, &UShooterCharacterMovement::OnHit);
	}
}
...
void UShooterCharacterMovement::OnComponentDestroyed(bool bDestroyingHierarchy)
{
	if (GetPawnOwner() != nullptr && GetPawnOwner()->GetLocalRole() > ROLE_SimulatedProxy)
	{
		...
		static_cast<AShooterCharacter*>(GetCharacterOwner())->GetCapsuleComponent()->OnComponentHit.RemoveDynamic(this, &UShooterCharacterMovement::OnHit);
	}
...
void UShooterCharacterMovement::TickComponent(float DeltaTime, enum ELevelTick TickType, FActorComponentTickFunction* ThisTickFunction)
{
	...
	{
		if (SprintKeyDown || SprintKeyDownToggle)
		{
...
void UShooterCharacterMovement::PhysCustom(float deltaTime, int32 Iterations)
{
	...
	UE_LOG(LogTemp, Warning, TEXT("%lf %lf"), Velocity.X, Velocity.Y);
...
void UShooterCharacterMovement::PhysWallRunning(float deltaTime, int32 Iterations)
{
	...
	{
		if (fabs(Velocity.X) < 100 && fabs(Velocity.Y) < 100)
		{
			EndWallRun();
			return;
		}
...
void UShooterCharacterMovement::SetSprinting(bool sprinting, bool sprintingToggle)
{
	SprintKeyDown = sprinting;
	SprintKeyDownToggle = sprintingToggle;
}
...
bool UShooterCharacterMovement::IsNextToWall(float vertical_tolerance)
{
	...
	FVector crossVector = WallRunSide == EWallRunSide::left ? FVector(0.0f, 0.0f, -1.0f) : FVector(0.0f, 0.0f, 1.0f);
	FVector traceStart = GetCharacterOwner()->GetActorLocation() + (WallRunDirection * 20.0f);
	...
	if (vertical_tolerance > FLT_EPSILON)
	{
		...
		{
			UE_LOG(LogTemp, Warning, TEXT("Sium"));
			return false;
		}
...
void UShooterCharacterMovement::OnHit(UPrimitiveComponent* HitComponent, AActor* OtherActor, UPrimitiveComponent* OtherComp, FVector NormalImpulse, const FHitResult& Hit)
{
	if (IsCustomMovementMode(ECustomMovementMode::CMOVE_WallRunning))
	{
		EndWallRun();
...
void FSavedMove_ShooterCharacter::SetMoveFor(ACharacter* Character, float InDeltaTime, FVector const& NewAccel, class FNetworkPredictionData_Client_Character& ClientData)
{
	...
	UShooterCharacterMovement* CharMove = static_cast<UShooterCharacterMovement*>(Character->GetCharacterMovement());
	if (CharMove)
	{
		// Copy values into the saved move
		SavedWantsToSprint = CharMove->WantsToSprint;
		SavedWallRunKeysDown = CharMove->WallRunKeysDown;
	}
}
...
void FSavedMove_ShooterCharacter::PrepMoveFor(class ACharacter* Character)
{
	...
	if (CharMove)
	{
		...
		CharMove->WantsToSprint = SavedWantsToSprint;
		CharMove->WallRunKeysDown = SavedWallRunKeysDown;
	}
}
